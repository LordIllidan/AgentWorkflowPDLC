...
- **angular-frontend:** osobne kroki `npm ci`, `npm run build`, `npm run test:ci` (Vitest, lockfile `package-lock.json`). Przy `CI=true` włączane są reportery **default**, **github-actions** (adnotacje przy pliku/linii w UI Actions) oraz **JUnit** do `reports/vitest-junit.xml`.
